...
local function getBlipColor(business)
	if isNpcBusiness(business) then
		return BLIP_COLORS.npc
	end

	if (tonumber(business.stock) or 0) <= 0 then
		return BLIP_COLORS.outOfStock
	end

	return BLIP_COLORS.owned
end

local function updateShopBlips()
	local success, businesses = pcall(function()
		return lib.callback.await('economyreworked:getBusinesses', false)
	end)
	if not success then return end

	for shopType, shopData in pairs(LOCATIONS) do
		for locationIndex, locationCoords in pairs(shopData.coords or {}) do
			local blip = Blips[shopType .. locationIndex]
			if blip then
				for _, business in ipairs(businesses or {}) do
					local businessCoords = business.coords
					if business.type == shopType and businessCoords then
						local x = locationCoords.x - businessCoords.x
						local y = locationCoords.y - businessCoords.y
						local z = locationCoords.z - businessCoords.z
						if (x * x) + (y * y) + (z * z) < 25.0 then
							SetBlipColour(blip, getBlipColor(business))
							break
						end
					end
				end
	...
	for shopID, storeData in pairs(LOCATIONS) do
		...
		for locationIndex, locationCoords in pairs(storeData.coords) do
			if not storeData.blip.disabled then
				...
				Blips[shopID .. locationIndex] = StoreBlip
			end
...
RegisterNetEvent('ef_shops:updateShopProducts', function()
	updateShopBlips()
end)
